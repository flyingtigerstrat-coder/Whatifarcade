...
1. Cloudflare dashboard → **Storage & Databases → KV → Create namespace** → name it anything (e.g. `ironlinesaves`). Copy its **ID**.
...
4. Copy the live URL (`https://ironlinesaves.<account>.workers.dev`) and hand it back to the builder —
...
## Verifying
```
curl https://ironlinesaves.<account>.workers.dev/v1/health
